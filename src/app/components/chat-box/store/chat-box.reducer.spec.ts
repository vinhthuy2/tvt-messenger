...
import { chatBoxFeature, initialChatBoxState } from '@app/components/chat-box/store/chat-box.reducer';
...
describe('chat-box reducer', () => {
  it('should return state correctly on conversationLoaded', () => {
    ...
    const action = ChatBoxActions.loadConversationSuccessfully({ conversation });

    const state = chatBoxFeature.reducer(initialChatBoxState, action);
    const expectedState: Conversation = { ...initialChatBoxState, ...conversation };
