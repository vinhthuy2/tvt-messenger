...
export const onSelectChat = createEffect(
  (actions$ = inject(Actions)) => {
    return actions$.pipe(
      ...
      map(({ chatId }) => ChatBoxActions.loadConversation({ conversationId: chatId })),
    );
  },
  {
    functional: true,
  ...
  (actions$ = inject(Actions), httpService = inject(HttpService)) => {
    return actions$.pipe(
      ...
      exhaustMap(({ conversationId }) => {
        return httpService.getConservationById(conversationId)
          .pipe(
            map((conversation) => ChatBoxActions.loadConversationSuccessfully({ conversation })),
