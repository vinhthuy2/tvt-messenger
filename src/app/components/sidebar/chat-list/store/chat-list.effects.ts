...
import { CreateConversationRequest } from '@core/types';
...
export const loadChatList = createEffect(
  (actions$ = inject(Actions), httpService = inject(HttpService)) =>
    actions$.pipe(
      ...
      exhaustMap(({ userId }) =>
        httpService.getConservationListByUserId(userId)
          .pipe(
            map((chatList) => ChatListActions.loadChatListSuccess({ chatList: chatList.items })),
  ...
  (actions$ = inject(Actions), store = inject(Store)) =>
    actions$.pipe(
      ...
      map(([{ friendId }, chatList]) => {
        const chat = chatList
          .find((c) => !c.isGroup && c.participantIds.includes(friendId));
...
export const createPrivateChat = createEffect(
  (actions$ = inject(Actions), store = inject(Store), httpService = inject(HttpService)) =>
    actions$.pipe(
      ...
      exhaustMap(([{ friendId }, userId]) => {
        ...
        return httpService.createConversation(conversationRequest)
          .pipe(
            map((chat) => ChatListActions.createPrivateChatSuccess({ conversation: chat })),
