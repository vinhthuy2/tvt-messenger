...
import { Conversation, ConversationSummary, CreateConversationRequest } from '@core/types';
...
describe('ChatListEffects', () => {
  ...
  describe('on ChatListEffects.loadChatList', () => {
    it('should call api to load chatList', () => {
      ...
      const expected = hot('a', {
        a: ChatListActions.loadChatListSuccess({ chatList: [mockConversationSummary1, mockConversationSummary2] }),
      });
  ...
  describe('on ChatListActions.findPrivateChatByFriendId', () => {
    it(
      'should be dispatch ChatListActions.selectChat when private chat exists',
      () => {
        ...
        expect(ChatListEffects.onFindPrivateChatByFriendId(actions$, store)).toBeObservable(expected);
      },
    );

    it(
      'should dispatch ChatListActions.createPrivateChat when private chat does not exist',
      () => {
        const friendId = 'friendId';
  ...
  describe('on ChatListActions.createPrivateChat', () => {
    ...
    it('should dispatch ChatListActions.CreateConversationSuccess when create successfully', () => {
      ...
      const conversation: Conversation = {
        ...
        participants: [userId, friendId].map(
          id => ({
            id,
            name: '',
            isOnline: true,
      ...
      const expected = hot('a', {
        a: ChatListActions.createPrivateChatSuccess({ conversation: mockConversationSummary }),
      });
      ...
      const obs = executeObservableWithHttpRequest(
        ...
        undefined,
        (req: HttpRequest<CreateConversationRequest>) => {
  ...
  describe('on ChatListActions.createdChatSuccessfully', () => {
    it('should dispatch ChatListActions.SelectChat when create successfully', () => {
      ...
      actions$ = hot('a', {
        a: ChatListActions.createPrivateChatSuccess({ conversation: mapToConversationSummaryDto(conversation) }),
      });

      const expected = hot('(ab)', {
        a: ChatListActions.addToChatList({ conversation: mapToConversationSummaryDto(conversation) }),
        b: ChatListActions.selectChat({ chatId }),
      });

      expect(ChatListEffects.createdChatSuccessfully(actions$)).toBeObservable(expected);
    });
  });
});
