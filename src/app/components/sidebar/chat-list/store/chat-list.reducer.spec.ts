...
import { mockConversation1, mockConversationList } from '@testing/mocks/conversations.mock';
import { ChatListActions } from './chat-list.actions';
import { ChatListFeature, chatListFeatureKey, ChatListState } from './chat-list.reducer';
...
describe(`${chatListFeatureKey} chat list reducer`, () => {
  it('should update the state with the provided chat list on ChatListActions.loadChatListSuccess', () => {
    ...
    const chatList: ConversationSummary[] = mockConversationList.map(mapToConversationSummaryDto);
  ...
  it('should not change the chat list when selecting a chat on ChatListActions.selectChat', () => {
    ...
    expect(state.chatList).toEqual(mockConversationList.map(mapToConversationSummaryDto));
  });
  ...
  it('should add new chat to the chat list on ChatListActions.addChat', () => {
    ...
    const newChat: ConversationSummary = mapToConversationSummaryDto(mockConversation1);
